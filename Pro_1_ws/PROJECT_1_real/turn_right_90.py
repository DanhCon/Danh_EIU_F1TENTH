...
class TurnRight90Node(Node):
    def __init__(self):
        super().__init__('turn_right_90_node')

        self.declare_parameter('target_deg', -90.0)     # Quay phải -90 độ
        ...
        self.get_logger().info("=== BẮT ĐẦU QUAY SANG PHẢI 90 ĐỘ ===")
    ...
    def control_loop(self):
        ...
        if rem_rad <= math.radians(0.5):
            ...
            self.get_logger().info(f"✅ ĐÃ HOÀN THÀNH QUAY PHẢI 90°! (Góc đạt: {current_deg:.2f}°)")
            return

        w = self.kp * rem_rad
        w = max(self.min_w, min(self.max_w, w))
        cmd.angular.z = -w  # Quay thuận chiều kim đồng hồ (Phải)
        self.cmd_pub.publish(cmd)

        self.get_logger().info(f"Đang quay phải: {current_deg:.1f}° / 90.0°", throttle_duration_sec=0.5)

def main(args=None):
    rclpy.init(args=args)
    node = TurnRight90Node()
    try:
        rclpy.spin(node)
