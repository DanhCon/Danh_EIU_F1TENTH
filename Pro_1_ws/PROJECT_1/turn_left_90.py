...
class TurnLeft90Node(Node):
    def __init__(self):
        super().__init__('turn_left_90_node')

        self.declare_parameter('target_deg', 90.0)      # Quay trái +90 độ
        ...
        self.get_logger().info("=== BẮT ĐẦU QUAY SANG TRÁI 90 ĐỘ ===")
    ...
    def control_loop(self):
        ...
        if rem_rad <= math.radians(0.5):
            ...
            self.get_logger().info(f"✅ ĐÃ HOÀN THÀNH QUAY TRÁI 90°! (Góc đạt: {current_deg:.2f}°)")
            return

        w = self.kp * rem_rad
        w = max(self.min_w, min(self.max_w, w))
        cmd.angular.z = w  # Quay ngược chiều kim đồng hồ (Trái)
        self.cmd_pub.publish(cmd)

        self.get_logger().info(f"Đang quay trái: {current_deg:.1f}° / 90.0°", throttle_duration_sec=0.5)
