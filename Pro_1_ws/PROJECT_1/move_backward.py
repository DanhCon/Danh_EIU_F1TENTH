...
class MoveBackwardNode(Node):
    def __init__(self):
        super().__init__('move_backward_node')

        self.declare_parameter('distance', 1.0)          # Khoảng cách lùi (m)
        ...
        self.get_logger().info(f"=== BẮT ĐẦU LÙI LẠI {self.target_dist:.2f}M ===")
    ...
    def control_loop(self):
        ...
        if rem_dist <= 0.015:
            ...
            self.get_logger().info(f"✅ ĐÃ HOÀN THÀNH LÙI LẠI! (Quãng đường: {moved_dist:.3f}m)")
            return

        v = self.kp * rem_dist
        v = max(self.min_v, min(self.max_v, v))
        cmd.linear.x = -v  # Dấu âm để chạy lùi
        self.cmd_pub.publish(cmd)

        self.get_logger().info(f"Đang lùi: {moved_dist:.2f}m / {self.target_dist:.2f}m", throttle_duration_sec=0.5)

def main(args=None):
    rclpy.init(args=args)
    node = MoveBackwardNode()
    try:
        rclpy.spin(node)
