...
public:
    MPPIController() : Node("mppi_real_controller_node") {
        ...
        try {
            ...
            csv_log.open(ss.str());
            if (csv_log.is_open()) {
                csv_log << "time,x,y,theta,v_cur,target_v,steer_cmd,v_cmd,exec_time_ms,is_stopped,is_stuck,front_blocked,odom_delay,lidar_delay,best_track_cost,best_obs_cost,collision_rate,cte,heading_err,curvature,steer_raw,min_cost,avg_cost,min_front_dist,obs_cnt\n";
...
private:
    ...
    static constexpr double EMERGENCY_HALF_WIDTH          = 0.3;  // Nửa chiều rộng vùng phanh khẩn cấp (m)
	...
	double w_obs      = 180.0;  // Né chướng ngại vật (Tăng lên 250.0 để ưu tiên né tránh cao hơn)
    ...
    double alpha_s = 0.70;  // Hệ số EMA góc lái:   nhỏ → mượt hơn, trễ hơn (giảm về 0.40 để hết chattering)
    ...
    void control_loop() {
        ...
        // [TEST MODE] Tắt né tránh: xóa vật cản khỏi cost MPPI (obs_pts rỗng →
        // cost obs = 0). VẪN giữ log thống kê corridor để đối chiếu debug.
        ...
        // [TEST MODE] Chỉ kích hoạt khi bật né tránh. Nếu tắt, chỉ log số liệu
        // min_front_dist để biết hành lang phía trước đang trống hay có tường.
        ...
        // --- Log 7 [TEST MODE]: Chẩn đoán bám đường (real-time 10 Hz) ---
        //   cte   : cross-track error tổng = khoảng cách xe → waypoint gần nhất (lý tưởng ≈ 0)
        //   dx/dy : lệch riêng từng trục (dx lệch dọc đường, dy lệch ngang)
        //   herr  : lệch hướng xe so với hướng waypoint (lý tưởng ≈ 0)
        //   curv  : độ cong lớn nhất phía trước (speed_factor nằm trong [0,1])
        //   tgt_v : tốc độ mục tiêu sau curvature + giới hạn gia tốc
        ...
        // --- 4.8 Kiểm tra front_blocked & Watchdog ---
        ...
        if (obs_stamp.nanoseconds() != 0 && !obs_fresh) {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000, 
                "[CẢNH BÁO] LiDAR bị trễ %.2fs (>%.1fs). Bộ lọc an toàn tạm tắt!", obs_age, LIDAR_TIMEOUT_S);
        }

        // Nếu KHÔNG dùng CBF (enable_cbf == false): Dùng cơ chế phanh khẩn cấp & lùi xe cũ
        if (!enable_cbf && obs_fresh && now_s > front_blocked_cooldown_until) {
        ...
        bool is_stuck = false;
        if (!enable_cbf && !is_stopped && now_s > watchdog_cooldown_until
        ...
        // --- 4.10 Emergency Stop & Escape Maneuver (Chỉ chạy khi không dùng CBF) ---
        if (!enable_cbf && !is_stopped && (front_blocked || is_stuck)) {
            is_stopped    = true;
        ...
        if (!enable_cbf && is_stopped && now_s > stop_end_time) {
            is_stopped                   = false;
            is_stuck_timer_active        = false;
        ...
        // Giới hạn vận tốc MPPI sampling: [0, target_v] khi bình thường, [escape_speed, 0] khi stop (lùi)
        double v_min = (enable_cbf || !is_stopped) ? 0.0 : escape_speed;
        double v_max = (!enable_cbf && is_stopped) ? 0.0 : target_v;
        if (!enable_cbf && is_stopped) target_v = escape_speed;
        double w_prog_eff = (!enable_cbf && is_stopped) ? 0.0 : w_progress;
        ...
        auto t_cost_start = std::chrono::high_resolution_clock::now();
        ...
        for (int n = 0; n < num_samples; n++) {
            ...
            for (int t = 0; t < horizon; t++) {
                ...
                // [TEST MODE] Tắt né tránh → bỏ hẳn vòng lặp vật cản (tiết kiệm CPU)
                ...
                if (enable_obstacle_avoidance) {
                    ...
                    if (min_abs_d < COLLISION_DISTANCE_M) {
                        obs += collision_cost; // Va chạm trực tiếp
                        collided = true;
                    }
                }
        ...
        // --- 4.15 CBF Safety Shield (Tham khảo từ code_chay_cbf_thuong.py) ---
        double final_v     = last_ema_v;
        double final_steer = last_ema_steer;
        ...
        if (enable_cbf && obs_fresh) {
            ...
            for (const auto& pt : raw_obs) {
                // Chỉ xét chướng ngại vật phía trước xe (x > 0.05m)
                ...
                if (angle_deg <= cbf_fov_cutoff_deg) {
                    ...
                    double cos_phi = pt.x / r_i; // cos(phi_i)

                    // Điều kiện rào chắn CBF: v * cos(phi_i) <= gamma * (r_i - d_min)
            ...
            // Nếu r_i <= cbf_d_min, limit_i <= 0 -> v_cbf_max <= 0 -> v_safe = 0.0 (Dừng hoàn toàn tại chỗ)
        ...
        publish_drive(final_v, final_steer);
        publish_best_trajectory(x, y, th);

        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 100,
            "MPPI | cost=%6.0f | curv=%.2f | tgt=%.2f | v=%.2f | ema_v=%.2f | ema_s=%5.2f | raw_s=%5.2f | blk=%d stk=%d stp=%d | mode=%s",
            min_cost, max_c, target_v, vc, last_ema_v, last_ema_steer, nominal_control[0].steer,
            front_blocked, is_stuck, is_stopped,
        ...
        // Tìm index của mẫu tốt nhất
        int best_idx = std::distance(costs_buf.begin(), std::min_element(costs_buf.begin(), costs_buf.end()));

        // Tính toán chi tiết các thành phần điểm của mẫu tốt nhất
        double best_trk = 0.0, best_hdg = 0.0, best_spd = 0.0, best_smo = 0.0, best_obs = 0.0, best_terminal = 0.0;
        {
            double px = x, py = y, pth = th;
            double pv_prv = nominal_control[0].v;
            double ps_prv = nominal_control[0].steer;
            for (int t = 0; t < horizon; t++) {
                ...
                double herr = normalize_angle(pth - local_hdgs[min_wi]);
                best_hdg += herr * herr;
                best_spd += (pv - target_v) * (pv - target_v);
                if (t > 0) best_smo += (ps - ps_prv) * (ps - ps_prv) + (pv - pv_prv) * (pv - pv_prv);
                pv_prv = pv; ps_prv = ps;
                double max_pen = 0.0, min_abs_d = 999.0;
                if (enable_obstacle_avoidance) {
                    for (const auto& o : obs_pts) {
                    ...
                    best_obs += max_pen;
                    if (min_abs_d < COLLISION_DISTANCE_M) best_obs += collision_cost;
                }
                if (t == horizon - 1) {
                    double wh = is_stopped ? W_HEADING_STOPPED : w_heading;
                    best_terminal += TERMINAL_COST_MULTIPLIER * w_track * min_d2 + TERMINAL_COST_MULTIPLIER * wh * herr * herr;
                    ...
                    best_terminal += w_prog_eff * prog_v;
                }
            }
        }
        ...
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
            "[MPPI COST] Min (Best): %.1f | Avg: %.1f | Max: %.1f | w_sum=%.2e | effective=%.1f%%",
            min_cost, avg_cost, max_cost, w_sum,
            (w_sum > 0) ? (1.0 / w_sum) * 100.0 : 0.0);

        // --- Log 6: Chi tiết cơ cấu điểm Best Trajectory & Tỷ lệ Va chạm ---
        double collision_rate = (double)collision_samples / num_samples * 100.0;
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
            "[MPPI DIAG] Best breakdown: Track: %.1f, Heading: %.1f, Speed: %.1f, Smooth: %.1f, Obs: %.1f, Terminal: %.1f",
            w_track * best_trk, w_heading * best_hdg, w_speed * best_spd, w_smooth * best_smo, w_obs * best_obs, best_terminal);
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
            "[MPPI DIAG] Collision Rate: %.1f%% (%d/%d samples collided)",
            collision_rate, collision_samples, num_samples);
        ...
        if (csv_log.is_open()) {
            csv_log << now_s << "," << x << "," << y << "," << th << "," << vc << "," 
                    ...
                    << w_track * best_trk << "," << w_obs * best_obs << "," << collision_rate << ","
                    << cte << "," << herr_now << "," << max_c << "," << nominal_control[0].steer << ","
                    << min_cost << "," << avg_cost << "," << min_front_dist << "," << obs_cnt << "\n";
        }

        // --- Log 8 [PERF]: Thời gian từng khâu để tìm "nút thắt cổ chai" ---
        //   Budget: 50ms/vòng (20Hz). Nếu total > 50ms → MPPI bị trễ, cần xem khâu nào.
        double t_sample_ms = std::chrono::duration<double, std::milli>(t_cost_start - t_sample_start).count();
        double t_cost_ms   = std::chrono::duration<double, std::milli>(t_update_start - t_cost_start).count();
        double t_update_ms = std::chrono::duration<double, std::milli>(end_time - t_update_start).count();
        double t_other_ms  = elapsed.count() - t_sample_ms - t_cost_ms - t_update_ms;
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 500,
            "[PERF] total=%.2fms | sample=%.2f | cost=%.2f | update=%.2f | other=%.2f",
            elapsed.count(), t_sample_ms, t_cost_ms, t_update_ms, t_other_ms);
        if (elapsed.count() > 50.0) {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 500,
                "[PERF CẢNH BÁO] Vòng lặp vượt budget 50ms! (%.1fms) — MPPI sẽ chậm hơn 20Hz", elapsed.count());
        }
    }
