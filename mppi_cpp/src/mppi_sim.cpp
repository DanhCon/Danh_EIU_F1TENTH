...
private:
    ...
    // --- Tunable Parameters ---
    double target_speed_max = 5.5;      // Toc do toi da
    double min_speed_curve = 2.5;       // Toc do thap nhat khi bo cua gat
    ...
    double stuck_timer_thresh = 0.0;    // Thoi gian xac nhan xe bi ket (giay)
    double stop_timer_duration = 0.0;   // Thoi gian dung im khi gap vat can (giay)
    ...
    void control_loop() {
        ...
        for (const auto& v_obs : virtual_obstacles) {
            ...
            for (int i = 0; i < num_points; i++) {
                ...
                raw_obs_snapshot.push_back({v_obs.x + 0.25 * std::cos(angle), v_obs.y + 0.25 * std::sin(angle)});
            }
        }
        // -------------------------------------------
