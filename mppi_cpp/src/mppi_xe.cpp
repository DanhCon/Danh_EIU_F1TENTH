...
private:
    ...
    double w_track    = 12.0;    // Bám tâm đường (Giảm xuống 8.0 để xe dám lùi làn tránh vật cản)
    double w_heading  = 10.0;   // Song song đường đua (Đã giảm từ 35 xuống 15 để xe dám bẻ chéo đầu lách)
    ...
    // -- Tốc độ & Gia tốc --
    double target_speed_max = 3.5;  // Tốc độ tối đa (m/s)          [TUNE] 3.5 oke 
    double min_speed_curve  = 1.5;  // Tốc độ tối thiểu trong cua (m/s) 2.8 oke
    ...
    double escape_speed               = -0.8; // Tốc độ khi escape (m/s) - Đổi thành -0.8 m/s để xe LÙI ra khỏi vùng nguy hiểm
    ...
    void control_loop() {
        ...
        if (obs_fresh && now_s > front_blocked_cooldown_until) {
            ...
            // Duyệt trực tiếp qua raw_obs (hệ tọa độ local của xe) để check phanh khẩn cấp nhanh và chính xác nhất
            for (const auto& pt : raw_obs) {
                double dx_l = pt.x;
                double dy_l = pt.y;
        ...
        double v_min = is_stopped ? escape_speed : 0.0;
        double v_max = is_stopped ? 0.0 : target_v;
