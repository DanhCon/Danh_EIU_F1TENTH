...
public:
    MPPIController() : Node("mppi_real_controller_node") {
        ...
        try {
            ...
            csv_log.open(ss.str());
            if (csv_log.is_open()) {
                csv_log << "time,x,y,theta,v_cur,target_v,steer_cmd,v_cmd,exec_time_ms,is_stopped,is_stuck,front_blocked,odom_delay,lidar_delay,best_track_cost,best_obs_cost,collision_rate\n";
...
private:
    ...
    const std::string WAYPOINT_CSV_PATH =
        "/home/fablab_01/danh_pp_ws/f1tenth_waypoint.csv";
    ...
    // -- Tốc độ & Gia tốc --
    double target_speed_max = 1.5;  // Tốc độ tối đa (m/s)          [TUNE] 3.5 oke 
