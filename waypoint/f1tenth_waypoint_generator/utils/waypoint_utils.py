...
class CSVFile:
    def __init__(self, rclpy_node: Node):
        ...
        # Lấy đường dẫn share của package find_path (hoặc f1tenth_waypoint_generator)
        ...
        if not self.rclpy_node.has_parameter('path_to_csvFile'):
            self.rclpy_node.declare_parameter(
                name='path_to_csvFile',
                value=os.path.join(pkg_share, 'f1tenth_waypoint_generator', 'racelines')
            )
        
        if not self.rclpy_node.has_parameter('csvFile_name'):
            self.rclpy_node.declare_parameter(name='csvFile_name', value='f1tenth_waypoint.csv')

        try:
            csv_path = self.rclpy_node.get_parameter('path_to_csvFile').get_parameter_value().string_value
            csvFile_name = self.rclpy_node.get_parameter('csvFile_name').get_parameter_value().string_value
        except Exception as e:
            self.rclpy_node.get_logger().warn(f'Could not get parameters, using default. Error: {e}')
            csv_path = os.path.join(pkg_share, 'f1tenth_waypoint_generator', 'racelines')
            csvFile_name = 'f1tenth_waypoint.csv'

        # ✅ Ghép đường dẫn chuẩn
        self.path_to_csvFile = os.path.join(csv_path, csvFile_name)
