...
class MPPIController(Node):
    def __init__(self):
        super().__init__("mppi_real_controller_node")
        ...
        # Tăng noise steer lên 0.30 cho xe thật (do ma sát lớn, steering servo trễ)
        self.noise_sigma = np.array([1.0, 0.30])

        # Temperature λ: tương thích với cost scale sau chuẩn hóa
        self.lambda_ = 50.0

        # ── Giới hạn cơ giới (CẤU HÌNH XE THẬT - CHẬM ĐỂ AN TOÀN) ──
        self.max_speed = 2.5    # Giới hạn an toàn tuyệt đối cho lần test đầu
        self.min_speed = 0.0
        self.max_steer = 0.418  # Góc bẻ lái vật lý tối đa của xe F1TENTH thật (24 độ)
        ...
        self.robot_radius   = 0.35  # Vẫn giữ 35cm an toàn
        self.danger_radius  = 1.2   # Tăng vọt lên 1.2m vì hệ thống xe thật có độ trễ cơ học (VESC delay)

        # Tốc độ mục tiêu lớn nhất trên đường thẳng (m/s)
        self.target_speed = 2.0  # Chạy 2.0 m/s để test trước
    ...
    def _compute_cost(
        ...
        if len(obstacles_list) > 0:
            ...
            if n_obs_filtered > 0:
                ...
                # obs_cost kết hợp: phạt va chạm nhị phân (100.0) + số bước (10.0)
                # Dùng công thức gốc của mppi_nhat để ép xe né gắt vật cản
